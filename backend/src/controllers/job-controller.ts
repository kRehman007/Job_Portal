...
export const viewApplicants = async (req: Request, res: Response) => {
  const { id } = req.params;
...
export const applyForJob = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await sendEmailForApplying(
      userDetails?.fullName,
      userDetails?.email,
      JobDetail?.employer?.email,
      JobDetail?.title
    );
    // await redis.del("all-jobs");
    // await redis.del(`applied-jobs-${req.user.id}`);
...
export const updateApplicationStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (status !== "pending") {
      ...
      await sendEmailForAcceptingAndRejecting(
...
export const GetJobsOfEmployer = async (req: Request, res: Response) => {
